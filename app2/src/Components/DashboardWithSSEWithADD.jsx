...
function DashboardWithSSE() {
  ...
  const hasLoopedOnceRef = useRef(false);
      ...
      try {
        ...
        setImageDataArray((prevArray) => {
          ...
          const sortedArray = Array.from(updatedMap.values())
            .sort((a, b) => b.id - a.id)
            .slice(0, 2); // 5 images for loop

          // ✅ Set currentIndex to 0 to re-trigger animation logic
          setCurrentIndex(0);
  ...
  useEffect(() => {
    if (
      ...
      !isProcessing &&
      !showAd &&
      !loopPausedRef.current
    ) {
      const interval = setInterval(() => {
        setCurrentIndex((prevIndex) => {
          const newIndex = (prevIndex + 1) % imageDataArray.length;

          // ✅ Show ad only once per loop
          if (
            newIndex === 0 &&
            prevIndex === imageDataArray.length - 1 &&
            !hasLoopedOnceRef.current
          ) {
            loopPausedRef.current = true;
            hasLoopedOnceRef.current = true;
            setShowAd(true);
            // setShowAd(false);
            ...
            setTimeout(() => {
              ...
              hasLoopedOnceRef.current = false;
            }, 5000);

            return prevIndex; // Stop loop during ad
          }
          ...
          if (
            currentImage &&
            (!currentImage.screenshotSent || !currentImage.animationTriggered)
          ) {
            setIsProcessing(true);
  ...
  }, [imageDataArray, isProcessing, showAd]);
  ...
  // for trigger animation for first image
  useEffect(() => {
    const currentImage = imageDataArray[currentIndex];

    if (
      imageDataArray.length > 0 &&
      currentIndex === 0 &&
      currentImage &&
      !currentImage.animationTriggered &&
      !isProcessing &&
      !showAd
    ) {
      setIsProcessing(true);
      ...
      setImageDataArray((prevArray) =>
        prevArray.map((item, index) =>
          index === 0
            ? { ...item, animationTriggered: true, screenshotSent: true }
            : item
        )
      );
  ...
  }, [imageDataArray, currentIndex, isProcessing, showAd]);
            ...
            >
              ...
              {imageDataArray.length > 0 && (
                <>
                  ...
                  <div className="copyright-text">© Copyright Protected</div>
                </>
              )}
            </div>
          )}

...
function DashboardWithSSE() {
  ...
  if (!cameraContainerRef.current || !videoRef.current) return;
  ...
  // Function to draw and merge overlay
  const doCapture = () => {
    try {
      ctx.drawImage(video, 0, 0, width, height);

      domtoimage
        .toPng(container, { filter: (node) => node !== video })
        .then((overlayUrl) => {
  ...
  if ("requestVideoFrameCallback" in video) {
    ...
    // Fallback: small delay to let frame render
      ...
      {imageDataArray.length > 0 && (
        <>
          <div className="parent-container">
            <div className="user-info fade show">
          ...
          <div className="corner-wrapper">
            <img
              ...
              alt="Received"
              className="corner-image fade show"
            />
          </div>
